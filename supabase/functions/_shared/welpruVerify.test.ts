...
describe("requestWelpruVerify", () => {
  ...
  it("insert token แล้วเรียก sendPush พร้อม deep link ที่มี token", async () => {
    ...
    expect(pushArg.link).toContain("https://example.test/welpru-verify?token=");
...
describe("confirmWelpruVerify (token-only / magic-link)", () => {
  it("throw ConflictError ถ้า token ไม่พบ/ใช้แล้ว/หมดอายุ (update คืนแถวว่าง)", async () => {
    ...
    await expect(
      confirmWelpruVerify(client as never, { token: "bad-token" })
  ...
  it("throw ConflictError ถ้า staff_id ปัจจุบันของ user ไม่ตรงกับตอนขอยืนยัน", async () => {
    const { client } = makeClient((ctx) => {
      if (ctx.table === "welpru_link_tokens" && ctx.op === "update")
        return { data: [{ user_id: "u1", staff_id: "S001" }] };
      if (ctx.table === "users" && ctx.op === "select")
        return { data: { staff_id: "S999" } }; // เปลี่ยนไปแล้วหลังขอยืนยัน
      throw new Error(`unexpected: ${ctx.table}.${ctx.op}`);
    });
    await expect(
      confirmWelpruVerify(client as never, { token: "tok1" })
    ).rejects.toBeInstanceOf(ConflictError);
  });
  ...
  it("สำเร็จ: derive user_id จาก token → set welpru_verified_at + consent ให้ user_id ของ token", async () => {
    const calls: DbCallContext[] = [];
    const { client } = makeClient((ctx) => {
      calls.push(ctx);
      // token คืน user_id ของเจ้าของ (ไม่ได้รับ userId จากภายนอก)
      if (ctx.table === "welpru_link_tokens" && ctx.op === "update")
        return { data: [{ user_id: "u-from-token", staff_id: "S001" }] };
      if (ctx.table === "users" && ctx.op === "select") return { data: { staff_id: "S001" } };
    ...
    // consent ต้องผูกกับ user_id ที่มาจาก token (ไม่ใช่ค่าที่ client ส่ง)
    const consentInsert = calls.find((c) => c.table === "consent_records" && c.op === "insert");
    expect(consentInsert?.payload).toMatchObject({ user_id: "u-from-token", consent_type: "welpru_linking" });
  });
});
