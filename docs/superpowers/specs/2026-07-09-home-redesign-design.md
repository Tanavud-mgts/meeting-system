...
- **วันนี้ไฮไลต์**: พื้น `bg-nav-active-surface` + ตัวเลขวันที่เข้ม (ใช้ token ที่เพิ่งเพิ่ม) — ไฮไลต์เสมอแม้วันนั้น count=0
- หัวข้อการ์ด: "ตารางการจองสัปดาห์นี้" (`text-sm font-medium`)
- **กรณีทั้งสัปดาห์ว่าง (ทุกวัน count=0):** กริด 7 วันยัง**แสดงเสมอ** (วันนี้ยังไฮไลต์) + เพิ่มบรรทัดข้อความจางใต้กริด `text-xs text-text-muted` **"ยังไม่มีการจองในสัปดาห์นี้"** เพื่อสื่อว่าว่างจริง แยกจากสถานะ "กำลังโหลด" (Skeleton) และ "โหลดพัง" (ข้อความ error) — เงื่อนไข: แสดงเมื่อ `days.every(d => d.count === 0)` และไม่ได้อยู่ในสถานะ loading/error
